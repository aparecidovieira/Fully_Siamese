...
class Siamese_fpn(tf.keras.Model):
    def __init__(self, num_classes, input_shape=(None, None, None, 3), n_filters=64, **kwargs):
        ...
        self.conv4_0 = conv_block(8 * n_filters)
        ...
        self.conv4_1 = conv_block(8 * n_filters)

        
        self.bottleneck = conv_block(8 * n_filters)
        ...
        self.up_block_0 = conv_block(n_filters)
        self.up_block_1 = conv_block(2 * n_filters)
        self.up_block_2 = conv_block(4 * n_filters)
        self.up_block_3 = conv_block(8 * n_filters)

        self.final_tri = conv_block(n_filters)
    ...
    def call(self, inputs, training=True):
        ...
        x4_0 = max_pool()(self.conv4_0(x3_0, training=training))
        ...
        concat_x_4 = layers.concatenate([x4_0, x4_1], axis=-1)
        ...
        up_block3 = self.final_up3(bottleneck, training=training)
        up_block3 = layers.concatenate([up_block3, concat_x_3], axis=-1)
        up_block3 = self.up_block_3(up_block3)

        up_block2 = self.final_up2(up_block3, training=training)
        up_block2 = layers.concatenate([up_block2, concat_x_2], axis=-1)
        up_block2 = self.up_block_2(up_block2)

        up_block1 = self.final_up1(up_block2, training=training)
        up_block1 = layers.concatenate([up_block1, concat_x_1], axis=-1)
        up_block1 = self.up_block_1(up_block1)

        up_block0 = self.final_up0(up_block1, training=training) 
        up_block0 = layers.concatenate([up_block0, concat_x_0], axis=-1)
        up_block0 = self.up_block_0(up_block0)

        up_block0 = self.final_up_final(up_block0, training=training)

        out = self.final_conv(up_block0)
